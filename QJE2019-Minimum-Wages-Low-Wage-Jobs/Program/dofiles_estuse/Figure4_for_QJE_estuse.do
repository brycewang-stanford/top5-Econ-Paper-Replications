* ESTUSE COPY generated by Program/run_original.do documentation: identical to Program/dofiles/Figure4_for_QJE.do
* except that the reghdfe/est save lines are commented out (prefix '*ESTUSE*'), so the exhibit is rebuilt
* from the authors' shipped .ster files (Materials/author_outputs/estimates -> Results/estimates).
...
*ESTUSE* ${rtype`s'} `Y'countpc  ${treatafter} one if  year>=`b'  & cleansample==1 `if`Ycounter''   ${weight`Y'`b'} , a(${a`s'} ${controlbefore}  ${window} ${placebocontafter1} ${placebobefore1} ${windowpl1} ${placebocontafter2} ${placebobefore2} ${windowpl2} ${controlf}  ${control}  )  cluster(statenum)
*ESTUSE* est save "${estimates}`Y'`Ycounter'pl0after`limitsample'`limittreat'_`s'_`Y'_`b'_`temp1'_`temp2'_${Tmax}${Tmin}", replace 
...
*ESTUSE* ${rtype`s'} `Y'countpc ${placeboafter1} one if  year>=`b'  & cleansample==1 `if`Ycounter''   ${weight`Y'`b'} , a(${a`s'} ${controlbefore}  ${controlafter}  ${window}  ${placebocontafter2} ${windowpl2}  ${placebobefore2} ${placebobefore1} ${windowpl1} ${controlf}  ${control}  )  cluster(statenum)
*ESTUSE* est save "${estimates}`Y'`Ycounter'pl1after`limitsample'`limittreat'_`s'_`Y'_`b'_`temp1'_`temp2'_${Tmax}${Tmin}", replace 
...
*ESTUSE* ${rtype`s'} `Y'countpc ${placeboafter2} one if  year>=`b'  & cleansample==1 `if`Ycounter''   ${weight`Y'`b'} , a(${a`s'} ${controlafter}  ${controlbefore}  ${window} ${placebocontafter1}  ${placebobefore1} ${windowpl1} ${placebobefore2} ${windowpl2} ${controlf}  ${control}  )  cluster(statenum)
*ESTUSE* cap drop sample_indic
*ESTUSE* gen sample_indic = e(sample)
*ESTUSE* sum quarterdate if sample_indic==1, meanonly
*ESTUSE* estadd scalar sample_beg=r(min)
*ESTUSE* estadd scalar sample_end=r(max)
*ESTUSE* cap drop sample_indic
*ESTUSE* est save "${estimates}`Y'`Ycounter'pl2after`limitsample'`limittreat'_`s'_`Y'_`b'_`temp1'_`temp2'_${Tmax}${Tmin}", replace 
