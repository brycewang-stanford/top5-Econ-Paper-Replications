* ESTUSE COPY: identical to Program/dofiles/grouplevelanalysis_new_QJE.do except that the reghdfe and est save lines are
* commented out (prefix '*ESTUSE*'); estimates come from the authors' shipped .ster files.
...
*ESTUSE* ${rtype`s'} `vv'`vv2'countpc  ${treatafter} one if  year>=`b'  & wt`vv'`vv2'`b'>0 & cleansample==1 ${if`s'}  ${weight`vv'`vv2'`b'} , a(${a`s'}  ${control} ${controlbefore} ${controlf} ${window} )  cluster(statenum)
*ESTUSE* est save "${estimates}after`vv'`vv2'`limitsample'`limittreat'_`s'_`vv'`vv2'_`b'_`temp1'_`temp2'_${Tmax}${Tmin}", replace 
